  ...
  useEffect(() => {
    ...
    const handleMessage = async (event: MessageEvent) => {
      ...
      if (message.type === 'BILAN_SAVE_COMMITTED') {
        setBridgeMessage('Bilan enregistré. Vous pouvez maintenant télécharger le rapport.')
        setTimeout(() => setBridgeMessage(''), 2500)
        return
      }

      if (message.type === 'BILAN_OUTPUT_DONE') {
        setBridgeMessage('Rapport généré. Mise à jour du crédit…')
      ...
      setSyncing(true)
      setBridgeMessage('Validation du crédit bilan…')
      ...
      try {
        const { data, error } = await supabase.rpc(
          'consume_bilan_credit',
          {
            ...
            p_client_name: message.clientName || 'Client',
            p_period: message.period || 'M0',
          }
        )

        if (error) {
          console.error('consume_bilan_credit:', error)
          sendResult(message.assessmentId, {
            ok: false,
            status: 'server_error',
            message: error.message || 'Erreur de validation du crédit.',
          })
          return
        }
        ...
        console.error('Bilan bridge:', unknownError)
        sendResult(message.assessmentId, {
          ok: false,
  ...
  return (
    ...
    >
      ...
      {(syncing || bridgeMessage) && (
        ...
        >
          {bridgeMessage || 'Validation en cours…'}
        </div>
      )}
    </div>
  )
