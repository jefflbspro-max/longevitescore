...
export default function Login({ onClose }: LoginProps) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: '#111116', border: '1px solid rgba(196,168,130,0.2)', borderRadius: 16, padding: '40px 36px', width: 380, maxWidth: '90vw' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <input
            ...
            style={{ background: 'rgba(196,168,130,0.06)', border: '1px solid rgba(196,168,130,0.2)', borderRadius: 8, padding: '12px 14px', color: C.ivoire, fontSize: 14, outline: 'none' }}
          />
          {error && <div style={{ color: C.rouge, fontSize: 12 }}>{error}</div>}
          {success && <div style={{ color: '#22c55e', fontSize: 12 }}>{success}</div>}
          <button type="submit" disabled={loading}
            style={{ background: C.or, color: C.noir, border: 'none', borderRadius: 8, padding: '13px', fontWeight: 700, fontSize: 14, cursor: 'pointer', marginTop: 4 }}>
